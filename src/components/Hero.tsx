...
const GithubCalendarWidget = dynamic(
  () => import("./GithubCalendarWidget").then((mod) => mod.GithubCalendarWidget),
  {
    ssr: false,
    loading: () => (
      <div className="h-[240px] w-full rounded-2xl border border-border/40 bg-muted/20 animate-pulse" />
    )
  }
);

const LeetCodeWidget = dynamic(
  () => import("./LeetCodeWidget").then((mod) => mod.LeetCodeWidget),
  {
    ssr: false,
    loading: () => (
      <div className="h-[240px] w-full rounded-2xl border border-border/40 bg-muted/20 animate-pulse" />
...
export function Hero() {
  ...
  // Fail-safe load state in case of slow networks or cached images missing onload trigger
  useEffect(() => {
    if (bypass) {
    ...
    >
      ...
      <div className="container mx-auto px-6 relative z-10 w-full max-w-[84rem]">
        ...
        <div className="grid gap-6 sm:gap-8 lg:gap-24 lg:grid-cols-3 items-center justify-center">
          ...
          <motion.div
            initial={bypass ? false : { opacity: 0, y: 50 }}
            animate={heroImageLoaded ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
          ...
          >
            ...
            >
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[0.65rem] font-semibold font-mono bg-accent-foreground/10 text-accent-foreground">
                <span className="text-accent-foreground">import</span>{" "}
                <span className="text-foreground">{"{ Developer }"}</span>{" "}
                <span className="text-accent-foreground">from</span>{" "}
                <span className="text-primary">"nirajbava"</span>
              </div>
          ...
          <div className="order-1 lg:order-2 flex flex-col items-center justify-center scale-[0.98] sm:scale-100">
            ...
            >
              ...
              <motion.div
                initial={bypass ? false : { opacity: 0, y: 250, scale: 0.95 }}
                animate={
                  !heroImageLoaded
                    ? { opacity: 0, y: 250, scale: 0.95 }
                    : isPortraitHovered
                      ? {
                        opacity: 1,
              ...
              {isInView && floatingTechs.map((tech, index) => (
                <motion.div
                  key={`wrapper-${tech.name}`}
                  className={`absolute ${tech.pos} z-20 will-change-transform`}
                  animate={heroImageLoaded ? { y: [0, -10, 0] } : {}}
                  transition={{
                ...
                >
                  <motion.div
                    key={tech.name}
                    initial={bypass ? false : { opacity: 0, scale: 0 }}
                    animate={
                      !heroImageLoaded
                        ? { opacity: 0, scale: 0 }
                        : isPortraitHovered
          ...
          <motion.div
            initial={bypass ? false : { opacity: 0, y: 50 }}
            animate={heroImageLoaded ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
            transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1], delay: bypass ? 0 : 0.15 }}
